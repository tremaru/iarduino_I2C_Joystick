...
uint16_t iarduino_I2C_Joystick::getButton		(uint8_t type){																	//	Параметр:				type - тип получаемого состояния или собития.
			...
			//	Определяем возвращаемое значение:																				//
				switch(type){																									//
					case KEY_PUSHED		:	f=(val_REG_JOY_KEY & JOY_KEY_PUSHED  );	val_REG_JOY_KEY&=~JOY_KEY_PUSHED;	break;	//	Возвращаемым значениям является флаг  события   кнопки - «нажимается».
					case KEY_RELEASED	:	f=(val_REG_JOY_KEY & JOY_KEY_RELEASED);	val_REG_JOY_KEY&=~JOY_KEY_RELEASED;	break;	//	Возвращаемым значениям является флаг  события   кнопки - «отпускается».
					case KEY_PRESSED	:	f=(val_REG_JOY_KEY & JOY_KEY_PRESSED );										break;	//	Возвращаемым значениям является флаг  состояния кнопки - «нажата».
					case KEY_TRIGGER	:	f=(val_REG_JOY_KEY & JOY_KEY_TRIGGER );										break;	//	Возвращаемым значениям является флаг  триггера  кнопки - «переключатель».
					case KEY_HOLD_05	:	f=(val_REG_JOY_KEY & JOY_KEY_HOLD_05 );										break;	//	Возвращаемым значениям является флаг  состояния кнопки - «удерживается» дольше 0,5 сек.
					case KEY_HOLD_10	:	f=(val_REG_JOY_KEY & JOY_KEY_HOLD_10 );										break;	//	Возвращаемым значениям является флаг  состояния кнопки - «удерживается» дольше 1,0 сек.
					case KEY_HOLD_20	:	f=(val_REG_JOY_KEY & JOY_KEY_HOLD_20 );										break;	//	Возвращаемым значениям является флаг  состояния кнопки - «удерживается» дольше 2,0 сек.
					case KEY_CHANGED	:	f=(val_REG_JOY_KEY & JOY_KEY_CHANGED );	val_REG_JOY_KEY&=~JOY_KEY_CHANGED;	break;	//	Возвращаемым значениям является флаг  события   кнопки - «состояние изменилось».
					case KEY_TIME_PRESSED :	f=(val_REG_JOY_KEY & JOY_KEY_PRESSED )? (uint16_t)data[1]*100:0;			break;	//	Возвращаемым значениям является время удержания кнопки в миллисекундах.
				}																												//
